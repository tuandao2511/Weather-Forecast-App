...
class RemoteWeatherDataSource private constructor(val remoteWeatherService: RemoteWeatherService
) : SunshineDataSource{
...
//        provideRetrofit()?.requestWeatherForecast(query, mode, units, cnt)
    ...
    override fun getForecasts(locationQuery: String, callback: SunshineDataSource.LoadForecastsCallback) {
        val disposable = remoteWeatherService?.requestWeatherForecast(locationQuery,format,units,ctn)?.
